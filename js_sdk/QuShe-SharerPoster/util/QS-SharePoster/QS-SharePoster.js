...
function returnPromise(obj) {
    ...
    return new Promise(async (rs, rj) => {
        try {
            ...
            if (drawArray) {
                if (typeof drawArray == 'function') drawArray = drawArray(params);
                if (_app.isPromise(drawArray)) drawArray = await drawArray;
                for (let i = 0; i < drawArray.length; i++) {
                    let drawArrayItem = drawArray[i];
                    switch (drawArrayItem.type) {
                        ...
                        case 'text':
                            ...
                            break;
                        case 'qrcode':
                            if (drawArrayItem.image) drawArrayItem.image = await _app.downloadFile_PromiseFc(drawArrayItem.image);
                            break;
                        case 'custom':
                            break;
                        default:
        ...
        try {
            ...
            setTimeout(() => {
                Context.draw(typeof reserve == 'boolean' ? reserve : false, () => {
                    ...
                    let setObj = {};
                    if (setCanvasToTempFilePath && typeof setCanvasToTempFilePath == 'function') setObj = setCanvasToTempFilePath(bgObj, type);
                    let canvasToTempFilePathFn;
                    // #ifdef H5
                    canvasToTempFilePathFn = function () {
                        _app.hideLoading();
                        rs({
                            tempFilePath: document.querySelector(`uni-canvas[canvas-id=${posterCanvasId}]>canvas`).toDataURL('image/jpeg', setObj.quality || 0.8)
                        });
                    };
                    // #endif
                    // #ifndef H5
...
function setTextFn(Context, textItem) {
    ...
    if (textItem.text && typeof textItem.text == 'string' && textItem.text.length > 0) {
        ...
        let infoCallBackObj = {};
        if (textItem.infoCallBack && typeof textItem.infoCallBack === 'function') infoCallBackObj = textItem.infoCallBack(textLength);
        textItem = {
            ...textItem,
            textLength,
            ...infoCallBackObj
