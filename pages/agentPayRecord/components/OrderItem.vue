<template>
    <view class="order-item" @tap="handleClick">
        ...
        <view class="products">
            <view class="product-item" v-for="(item, index) in order.skus" :key="index">
                ...
                <view class="price-count gray">
                    <view class="price">¥{{ item.price | priceToFixed }}</view>
                    <view class="count">x{{ item.total }}</view>
                </view>
            </view>
        </view>
        ...
        <view class="close-time red" v-if="closeTimeVisible">{{ hours }}:{{ minutes }}:{{ seconds }} 后关闭订单</view>
