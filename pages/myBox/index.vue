	...
	<view class="container">
		<view class="header-tabs">
			<view class="tab-item" v-for="(item, index) in typeTextList" :class="{active:current==index}"
				:data-current="index" @tap="currentChange" :key="index">
				<text class="text">
					{{item}}
					...
					<scroll-view scroll-y @scrolltolower="scrolltolower">
						<view class="order-list" v-if="dataList && dataList != ''">
							<PackageSku @check="checkItem(order)" :selectType="selectType" :isSelectMode="isSelectMode"
								...
								:order="order" :key="order.uuid" @action="actions" @locked="locked"></PackageSku>
		...
		<view class="footer-c" v-if="current == 0 && !orderConfig.is_hide_deliver_btn">
			<template v-if="!isSelectMode">
				<button class="select-box-btn enter-btn bg-orange" hover-class="hover" @tap="enterSelectMode('deliver')"
					v-if="!orderConfig.is_hide_deliver_btn">
					选择发货
				</button>

				<button class="select-box-btn enter-btn bg-cancel" style="margin-right: 50rpx;" hover-class="hover"
					@tap="enterSelectMode('return_sale')" v-if="orderConfig.is_return_sale_enable">
					选择回收
				</button>

				<button class="select-box-btn enter-btn bg-cancel" style="margin-right: 50rpx;" hover-class="hover"
					@tap="enterSelectMode('exchange')" v-if="exchangeConfig.is_enabled">
					选择置换
				</button>

				<button class="select-box-btn enter-btn bg-orange" hover-class="hover" @tap="enterSelectMode('resale')"
					v-if="marketConfig.is_enabled">
					批量挂售
				</button>
			...
			<template v-else>
				...
				<button class="select-box-btn bg-orange" hover-class="hover" @tap="selectOrSubmit"
					v-if="selectType === 'deliver'">
					提交发货 (已选{{selectedIds.length}}件)
				</button>
				<button class="select-box-btn bg-orange" hover-class="hover" @tap="batchReturnSale"
					v-if="selectType === 'return_sale'">
					确认回收 (已选{{selectedIds.length}}件)
				</button>
				<button class="select-box-btn bg-orange" hover-class="hover" @tap="batchResale"
					v-if="selectType === 'resale'">
					确认挂售 (已选{{selectedIds.length}}件)
				</button>
				<button class="select-box-btn bg-orange" hover-class="hover" @tap="batchExchange"
					v-if="selectType === 'exchange'">
		...
		<ReturnSalePopup @cancel="isShowReturnSalePopup = false" @refresh="refresh" :packageSku="currentItem"
			:uuid="currentUuid" v-if="isShowReturnSalePopup"></ReturnSalePopup>

		<ResalePopup @cancel="hideResalePopup" @refresh="refresh" :packageSku="currentItem" :uuid="currentUuid"
			v-if="isShowResalePopup"></ResalePopup>
		...
		<view class="mask" @tap="closepopup()" @touchmove.stop v-if="isReturnSaleSuccess">
			<view class="mask-content" @tap.stop>
				...
				<view class="title"><text class="new-iconfont icon-check"></text>{{selectType === 'return_sale' ? '回收' : '返售'}}成功</view>
