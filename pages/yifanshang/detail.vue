	...
	<view class="scope-container">
		<view class="content">
			...
			<view class="topbox">
				<view class="topimgbox">
					...
					<view class="topimgtext">
						...
						<view class="changebox" @tap="showRoomPopup"><text
								class="new-icon icon-express-package"></text>换箱</view>
					</view>
				</view>
				...
				<view class="topmainbox">
					...
					<view class="topmainboxb">
						<view class="textbox" @tap="showTips"><text class="new-icon icon-a-1"></text>规则说明</view>
						<view class="textbox" @tap="totabbar('/pages/myBox/index')"><text
								class="new-icon icon-jineqiandaiyueshangjin"></text>我的赏袋</view>
					</view>
				</view>
			</view>
			...
			<view class="body animated" :class="{bounceOutRight: startMoving, bounceInLeft: !startMoving}">
				<view class="title-c">
					<view class="stock-c" v-if="info.room && info.room.num">第{{info.room.num | fixNumber}}箱
						当前剩余{{info.room.stock}}/{{info.room.total}}张</view>
				</view>
				<view class="sku-list">
					<view class="skutitlebox">
						...
						<image src="../../static/spyla.png" mode="" class="skuspyla skuimga" v-if="skutype == 1">
						</image>
						<image src="../../static/csjla.png" mode="" class="skucsjla skuimga" v-else-if="skutype == 2">
						</image>
					</view>
					<view class="item" v-for="(item, index) in info.skus" :key="item.id" @tap="showSkuPopup"
						...
						<view class="bottomflex">
							<view class="display-price" style="border-right: 1px solid #ccc;">
								<view class="value">
									<text style="font-weight: bold;" v-if="item.display_money_price">
										￥{{item.display_money_price / 100}}
									</text>
		...
		<view class="footer-c">
			...
			<view class="btn-c">
				<image :class="{'gray-image': !isEnableBtn(1)}" src="/static/x1.png" mode="widthFix" class="btn"
					@tap="buy(1)"></image>
				<image :class="{'gray-image': !isEnableBtn(3)}" src="/static/x3.png" mode="widthFix" class="btn"
					@tap="buy(3)"></image>
				<image :class="{'gray-image': !isEnableBtn(10)}" src="/static/x10.png" mode="widthFix" class="btn"
					@tap="buy(10)"></image>
				<image :class="{'gray-image': !isEnableBtn(0)}" src="/static/all.png" mode="widthFix" class="btn"
					@tap="buy(0)"></image>
			</view>
		</view>
		...
		.topmainbox {
			...
			.topmainboxa {
				...
				.titletext {
					...
					margin-right: 30rpx;
					color: #fff;
					text-shadow: 1px 1px 3px #b2b2f8, -1px 1px 3px #b2b2f8, -1px -1px 3px #b2b2f8, 1px -1px 3px #b2b2f8;
				}
			}
