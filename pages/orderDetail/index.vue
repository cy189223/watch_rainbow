<template>
    <view class="container" v-if="order.uuid">
        ...
        <view class="products-c">
            <SkuItem class="sku" :info="item" v-for="(item, index) in skus" :key="index"></SkuItem>
        </view>
        ...
        <view class="price-wrap padding-h">
            ...
            <view class="list-item" v-if="order.cover_discount">
                ...
                <view class="arrow-right">-¥{{ order.cover_discount | priceToFixed }}</view>
            </view>
            <view class="list-item">
                <view class="body">优惠券</view>
                <view class="arrow-right">-¥{{ order.coupon_discount | priceToFixed }}</view>
            </view>
            <view class="list-item">
                <view class="body">
                    {{ $money() }}
                </view>
                <view class="arrow-right">-¥{{ order.redpack_discount | priceToFixed }}</view>
            </view>
            <view class="list-item" v-if="order.score_discount">
                <view class="body">{{ scoreAlias }}抵扣</view>
                <view class="arrow-right">{{ order.score_used }}{{ scoreAlias }} &nbsp -¥{{ order.score_discount | priceToFixed }}</view>
            </view>
            <view class="list-item">
                <view class="body">运费</view>
                <view class="arrow-right" v-if="order.carriage_type === 2">到付</view>
                <view class="arrow-right" v-else>+¥{{ order.carriage | priceToFixed }}</view>
            </view>
        ...
        <view class="meta-c padding-h">
            <view class="list-item">
                ...
                <view class="arrow-right">
                    {{ order.number }}
                    <text style="color: #1890ff; margin-left: 10rpx" @tap="setCopyText(order.number)">复制</text>
                </view>
            </view>
            ...
            <template v-if="deliverRecord">
                ...
                <view class="list-item" v-if="deliverRecord.express_number">
                    ...
                    <view class="arrow-right">
                        ...
                        <text style="color: #1890ff; margin-left: 10rpx" @tap="setCopyText(deliverRecord.express_number)">复制</text>
                    </view>
                </view>
        ...
        <view class="footer-actions safe-area-bottom">
            ...
            <template v-if="order.union_status == 'close_pending'">
                <button open-type="contact" @tap="openContact" class="btn bg-orange no-border-button bg-contact">
                    <text>联系客服</text>
                </button>
            </template>
        </view>
...
.footer-actions {
    ...
    .btn {
        ...
        &.bg-orange {
            background: rgba(105, 56, 182, 1) !important;
            color: #ffffff;
        }
