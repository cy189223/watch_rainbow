<template>
    <view
        ...
        :style="'height: 100vh; background: url(' + bgImage + '); background-size: 100% 100% ; background-position: center center;'"
        v-if="boxInfo"
    >
        ...
        <view class="footer safe-area-bottom" :class="{ small: isSmallDevice }">
            <view class="btns">
                ...
                <view class="button disable" hover-class="hover" @tap="showPreviewTips" v-if="boxInfo.is_presell && !boxInfo.presell_mode">预售中</view>
                <view class="button" hover-class="hover" @tap="buyNow" v-else>就买它</view>
            </view>
            <!-- <view class="tips"></view> -->
        </view>
        <OpenBoxPopup v-if="isOpenPopup" :order="order" @close="goBack" :boxImg="boxInfo.sku_3d_image"></OpenBoxPopup>
        ...
        <view class="tips-card-c" v-if="isShowExcludeCard">
            <view class="tips-content animated bounceInLeft">
                ...
                <view class="get-btn" @tap="useCard('exclude_box')" v-else>使用1张排除卡</view>
            </view>
        ...
        <view class="tips-card-c" v-if="isShowOpenCard">
            <view class="tips-content animated bounceInLeft">
                ...
                <view class="get-btn" @tap="useCard('show_box')" v-else>使用1张透视卡</view>
            </view>
...
import { getUsableCoupon } from '../../api/coupons.js';
...
export default {
    ...
    computed: {
        ...mapGetters(['userInfo']),
        defaultBoxImage() {
            return this.isInit ? 'https://api.caihongbox.com.cn/image/openbox.png' : '';
        },
        config() {
        ...
        freeTipLimit() {
            if (this.boxInfo.free_tips_limit === null) return 2;
        ...
        bgImage() {
            ...
            return this.boxInfo.room_bg_image || this.config.bg_image || 'https://api.caihongbox.com.cn/image/bg.png';
        },
        isDarkMode() {
    ...
    onShow() {
        ...
        uni.onAccelerometerChange((res) => {
            if (this.boxInfo.open_mode != 1) return;
            if (this.yaoyiyaoFlag) return;
            if (this.boxInfo.free_tips_limit === 0) return;
            ...
            if ((Math.abs(res.x) > level && Math.abs(res.y) > level) || (Math.abs(res.x) > level && Math.abs(res.z) > level)) {
...
.tips-card-c {
    ...
    .tips-content {
        background: url('https://api.caihongbox.com.cn/image/card-bg.png');
    ...
    .btns {
        ...
        .button {
            ...
            &:nth-child(2) {
                ...
                box-shadow: 0px 18rpx 30px 0px rgba(105, 56, 182, 0.2);
            }
