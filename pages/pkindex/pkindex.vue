<template>
    <view class="pagebox">
        ...
        <view class="sku-list">
            ...
            <view class="item" v-for="(item, index) in pkssku.list" :key="index">
                ...
                <view class="moneytext">
                    售价:{{ item.money_price == null ? '' : '￥' + (item.money_price / 100).toFixed(2) }}{{ item.money_price == null || item.score_price == null ? '' : '+'
                    }}{{ item.score_price == null ? '' : item.score_price + scoreAlias }}
                </view>
            ...
            <view class="list-scope">
                <view v-for="(item, index) in pkdata.users" :key="index" class="item-scope">
                    <view class="user-c" style="border-bottom: 1px dashed #eee; padding-bottom: 10rpx">
                        ...
                        <view class="weizhi" v-if="ispk">{{ item.type == 'A' ? 'A' : '通杀位' }}{{ item.type == 'A' ? item.seat_code + '号位' : '' }}</view>
        ...
        <view class="mainbox">
            <view class="userlistbox parta" :class="ispk ? '' : 'quanjushang'">
                ...
                <view class="moneynum" v-if="pkdata.activity && pkdata.activity.money_price != undefined">
                    入场：{{ pkdata.activity.money_price == 0 || !pkdata.activity.money_price ? '' : '￥' + (pkdata.activity.money_price / 100).toFixed(2)
                    }}{{ pkdata.activity.money_price == 0 || !pkdata.activity.money_price || pkdata.activity.score_price == 0 || !pkdata.activity.score_price ? '' : '+'
                    }}{{ pkdata.activity.score_price == 0 || !pkdata.activity.score_price ? '' : pkdata.activity.score_price + scoreAlias }}
                </view>
                <view class="blackbg" v-if="wintype != ''">
                    <block v-if="ispk">
                        <image src="https://api.caihongbox.com.cn/image/win.png" mode="aspectFit" v-if="wintype == 'A'"></image>
            ...
            <!-- <view class="userlistbox partb" style="display: flex; align-items: center; justify-content: center; flex-direction: column" v-if="ispk">
                <view
                    ...
                    v-for="(item, index) in pkdata.users"
                    v-if="item.type == 'B'"
                    :key="index"
                    @tap="topay(item)"
                >
                    <view class="usertextbox">
                        <view v-if="item.user && item.user.name" class="usernamebox">{{ item.user.name }}</view>
                        <view v-else class="joinbox" style="position: static; transform: translate(0, 0); width: 100%; box-sizing: border-box">通杀位</view>
                    </view>
                ...
                <view class="moneynum" v-if="pkdata.activity && pkdata.activity.other_money_price != undefined">
                    入场：{{ pkdata.activity.other_money_price == 0 || !pkdata.activity.other_money_price ? '' : '￥' + (pkdata.activity.other_money_price / 100).toFixed(2)
                        ...
                        pkdata.activity.other_money_price == 0 || !pkdata.activity.other_money_price || pkdata.activity.other_score_price == 0 || !pkdata.activity.other_score_price
                            ? ''
                            : '+'
                    }}{{ pkdata.activity.other_score_price == 0 || !pkdata.activity.other_score_price ? '' : pkdata.activity.other_score_price + scoreAlias }}
                </view>
                <view class="blackbg" v-if="wintype != ''">
        ...
        <PayCard
            v-if="isShowPayCard"
            :pkskus="pkdata.activity"
            :paytype="paytype"
            :payuuid="payuuid"
            :pktype="pktype"
            :payseat_uuid="payseat_uuid"
            :payjoin_type="payjoin_type"
            @success="paySuccess"
            @cancel="isShowPayCard = false"
        ></PayCard>
    </view>
</template>
