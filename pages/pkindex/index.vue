<template>
	<view class="container">
		...
		<HomeNavbar :title="page.title" searchType="all" :theme="scrollTop > 20 ? '#1d0051' : 'transparent'">
		</HomeNavbar>

		<view class="navlistbox">
			<view class="navbox" @tap="navtype = item.type" :class="navtype == item.type ? 'cur' : ''"
				...
				<view v-for="(item,index) in liushuilist.list" :key="index" style="padding-bottom: 5px;">
					...
					<view v-for="(ite,ind) in item.skus" :key="ind" class="mainbox">
						...
						<view class="flexbox">
							...
							<image v-if="ite.thumb && ite.thumb != ''" :src="ite.thumb" mode="aspectFill"
								class="imgbox"></image>
		...
		<view class="module-c" style="margin-top:0px; padding: 0 14px;" v-if="navtype == 'qjs'">
			<view class="list-scope list-grid2">
				<view class="item-container grid2" hover-class="hover" @click="clickyfsItem(item,navtype)"
					v-for="(item, index) in qjslist.list" :key="index">
					<view class="thumb-c">
						<image class="p-tag" :class="'location-' + item.image_tag.location" :src="item.image_tag.image"
		...
		<view class="module-c" style="margin-top:0px; padding: 0 14px;" v-if="navtype == 'ydd'">
			<view class="list-scope list-grid2">
				<view class="item-container grid2" hover-class="hover" @click="clickyfsItem(item,navtype)"
					v-for="(item, index) in yddlist.list" :key="index">
					<view class="thumb-c">
						<image class="p-tag" :class="'location-' + item.image_tag.location" :src="item.image_tag.image"
							v-if="item.image_tag"></image>
						<image mode="aspectFill" :src="item.thumb" class="thumb"></image>
...
<style lang="scss" scoped>
	...
	.navlistbox {
		...
		.navbox {
			...
			color: #fff;
			font-size: 16px;
			text-shadow: 1px 1px 0px #00d0ff, -2px 0px 0px #c33edc;
		}
