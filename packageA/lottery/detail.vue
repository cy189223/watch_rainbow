<template>
	<view class="container">
		<template v-if="isInit">
			...
			<view class="body"
				style="background: url('https://api.caihongbox.com.cn/image/activity-detail-bg.png'); background-size: 100% auto; background-repeat: no-repeat;">
				<view class="time">
					...
					<template v-else>
						<template v-if="info.emit_type === 'time'">
							<CountDown class="text" :start="info.emit_time" :end="info.end_at"></CountDown>后开奖
						</template>
				...
				<view class="block">
					<view class="block-title">
						...
						<view class="more"
							@tap="$tool.toPage(`/packageA/lottery/userList?uuid=${info.uuid}&type=all&actived=1`)">往期记录
						</view>
				...
				<template v-if="info.union_status == 'expired'">
					<view class="block">
						<view class="block-title">
							...
							<view class="more"
								@tap="$tool.toPage(`/packageA/lottery/userList?uuid=${info.uuid}&type=lucky`)">中奖名单
							</view>
							<text class="new-iconfont icon-arrow-right"></text>
						</view>
						<view class="user-list">
