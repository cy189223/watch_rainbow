...
class MpHtmlParser {
            ...
            for (var i = nodes.length, element; (element = nodes[--i]); ) {
                ...
                if (!element.c) {
                    ...
                    if (style) {
                        ...
                        if (style.indexOf('display') != -1 && ((tmp = style.match(/display\s*:\s*([^;]*)(?![\s\S]*?display)/)), tmp[1].indexOf('flex') == -1)) res += ';' + tmp[0];
    ...
    popNode(node) {
        ...
        if (node.pre) {
            this._whiteSpace = false;
            node.pre = undefined;
            for (var i = this._STACK.length; i--; ) if (this._STACK[i].pre) this._whiteSpace = true;
        }
        // 处理列表
        if (node.c) {
            if (node.name == 'ul') {
                ...
                if (floor != 1) for (i = node.children.length; i--; ) node.children[i].floor = floor;
        ...
        if (node.name == 'table') {
            ...
            function setBorder(elem) {
                ...
                for (var i = 0; i < (elem.children || []).length; i++) setBorder(elem.children[i]);
            }
            if (node.attrs.border || node.attrs.hasOwnProperty('cellpadding')) for (var i = 0; i < node.children.length; i++) setBorder(node.children[i]);
        }
        // 合并一些不必要的层，减小节点深度
        if (node.children.length == 1 && node.name == 'div' && node.children[0].name == 'div') {
            ...
            if (
                node.attrs.style.includes('padding') &&
                (node.attrs.style.includes('margin') || child.style.includes('margin')) &&
                node.attrs.style.includes('display') &&
                child.style.includes('display') &&
                !(node.attrs.id && node.attrs.id) &&
                !(node.attrs.class && child.class)
            ) {
