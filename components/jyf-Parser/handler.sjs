...
export default {
    getStyle: function (style, display) {
        ...
        if (style) {
            ...
            if (style.indexOf('float') != -1) res += style.match(getRegExp('float[^;]+(?![sS]*?float)'))[0];
            if (style.indexOf('margin') != -1) res += ';' + style.match(getRegExp('margin[^;]+', 'g')).join(';');
            if (style.indexOf('display') != -1 && ((tmp = style.match(getRegExp('displays*:s*([^;]*)(?![sS]*?display)'))), tmp[1].indexOf('flex') == -1)) res += ';' + tmp[0];
