...
<template>
    <view>
        ...
        <trees
            ...
            :nodes="nodes.length ? nodes : html && html.length && (html[0].name || html[0].type) ? html : []"
            :imgMode="imgMode"
            :loadVideo="loadVideo"
        />
        <!--#endif-->
...
export default {
    ...
    methods: {
        ...
        setContent(html, options, observed) {
            ...
            var style = '<style>';
            for (var item in config.userAgentStyles) style += item + '{' + config.userAgentStyles[item] + '}';
            for (var item in this.tagStyle) style += item + '{' + this.tagStyle[item] + '}';
            style += '</style>';
            html = style + html;
            if (this.rtf) this.rtf.parentNode.removeChild(this.rtf);
            ...
            for (var i = 0; i < imgs.length; i++) {
                ...
                if (this.domain && img.getAttribute('src')[0] == '/') {
                    if (img.getAttribute('src')[1] == '/') img.src = (this.domain.includes('://') ? this.domain.split('://')[0] : 'http') + ':' + img.getAttribute('src');
        ...
        Dom2Str(nodes) {
            ...
            for (var node of nodes) {
                if (node.type == 'text') str += node.text;
                else {
                    str += '<' + node.name;
                    for (var attr in node.attrs || {}) str += ' ' + attr + '="' + node.attrs[attr] + '"';
        ...
        setContent(html, options, observed) {
            ...
            if (!html) {
                ...
                if (this.useCache) {
                    var hash = Hash(html);
                    if (cache[hash]) this.nodes = cache[hash];
                    else {
                            ...
                            for (var j = component.nodes.length, item; (item = component.nodes[--j]); ) {
                                ...
                                if (item.name == 'img') {
                                    if (item.attrs.src && item.attrs.i) {
                                        // #ifndef MP-ALIPAY || APP-PLUS
                                        if (this.imgList.indexOf(item.attrs.src) == -1) this.imgList[item.attrs.i] = item.attrs.src;
                                ...
                                // #endif
                                // 设置标题
                                else if (item.name == 'title' && this.autosetTitle && item.children[0].type == 'text' && item.children[0].text)
                                    uni.setNavigationBarTitle({
        ...
        getText(whiteSpace = true) {
            ...
            const DFS = (node) => {
                if (node.type == 'text') return (text += node.text);
                    ...
                    if (
                        whiteSpace &&
                        (((node.name == 'p' || node.name == 'div' || node.name == 'tr' || node.name == 'li' || /h[1-6]/.test(node.name)) &&
                            text &&
                            text[text.length - 1] != '\n') ||
                            node.name == 'br')
                    )
                        text += '\n';
                    if (node.children) for (var i = 0; i < node.children.length; i++) DFS(node.children[i]);
                    if (
                        whiteSpace &&
                        (node.name == 'p' || node.name == 'div' || node.name == 'tr' || node.name == 'li' || /h[1-6]/.test(node.name)) &&
                        text &&
