    ...
    <view style="display: inherit; white-space: inherit">
        <block v-for="(item, index) in nodes" v-bind:key="index">
            ...
            <rich-text
                v-if="item.name == 'img'"
                :id="item.attrs.id"
                :style="'text-indent:0;' + item.attrs.containStyle"
                :nodes="[item]"
                :data-attrs="item.attrs"
                @tap="previewEvent"
            />
            <!--#endif-->
            <!--文本-->
            <!--#ifdef MP-WEIXIN || MP-KUAISHOU || MP-QQ || APP-PLUS-->
            ...
            <ad v-else-if="item.name == 'ad'" :unit-id="item.attrs['unit-id']" :class="item.attrs.class || ''" :style="item.attrs.style || ''" @error="adError" />
            <!--#endif-->
            <!--#ifdef MP-BAIDU-->
            <ad
                v-else-if="item.name == 'ad'"
                :appid="item.attrs.appid"
                :apid="item.attrs.apid"
            ...
            <!--#endif-->
            <!--#ifdef APP-PLUS-->
            <ad v-else-if="item.name == 'ad'" :adpid="item.attrs.adpid" :class="item.attrs.class || ''" :style="item.attrs.style || ''" @error="adError" />
            <!--#endif-->
            <!--列表-->
            ...
            <!--#ifdef MP-WEIXIN || MP-QQ || MP-ALIPAY || APP-PLUS-->
            <rich-text
                v-else-if="!(item.c || handler.isInlineTag(item.name) || item.continue)"
                :id="item.attrs.id"
                :class="'__' + item.name"
            ...
            <!--#endif-->
            <!--#ifdef MP-BAIDU || MP-TOUTIAO || MP-KUAISHOU -->
            <rich-text v-else-if="!(item.c || item.continue)" :id="item.attrs.id" :style="item.attrs ? item.attrs.containStyle : ''" :nodes="[item]" />
            <!--#endif-->
            <!--#ifdef MP-ALIPAY-->
    ...
    methods: {
        ...
        videoError(e) {
            if (!this.loadSource(e.currentTarget) && this._top)
                this.triggerError('video', e.currentTarget, e.detail.errMsg, undefined, uni.createVideoContext(e.currentTarget.id, this));
        },
        audioError(e) {
