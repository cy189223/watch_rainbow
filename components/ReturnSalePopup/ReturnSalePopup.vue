<template>
	<view class="mask" @tap="cancel" @touchmove.stop>
		<view class="mask-content" @tap.stop v-if="isInit && !isReturnSaleSuccess">
			...
			<view class="title">确认回收给平台吗?</view>
			<view class="sku-list" scroll-y v-if="skus.length <= 6">
				<view class="sku-c" v-for="(item, index) in skus" :key="index">
			...
			<view class="money-c">
				<view class="sub-title">当前折价率为{{info.return_sale_ratio}}，回收后你将获得:</view>
				...
				<view class="money item" v-if="info.return_redpack">
					...
					<view class="type">(退回{{$money()}})</view>
				</view>
				<view class="redpack item">
					<view>{{scoreAlias}}</view>
					<view class="number score">{{info.return_score}}</view>个
					<view class="type">(实时退回)</view>
				</view>
				<view class="tips">抽到{{scoreAlias}}、优惠券、{{$money()}}等虚拟礼品不可回收</view>
			</view>

			<view class="btn-c">
				<button class="btn cancel bg-purple" @tap="cancel">再想想</button>
				<button class="btn submit " @tap="submit">确认回收</button>
			</view>
		</view>
		<view class="mask-content" @tap.stop v-else-if="isReturnSaleSuccess">
			<text class="new-iconfont icon-close close-btn" @tap.stop="cancel"></text>
			<view class="title"><text class="new-iconfont icon-check"></text>回收成功</view>
			<!-- <view class="sku-c">
			...
			<view class="btn-c">
				...
				<button class="btn submit" @tap="toPage('/pages/myScore/index')">{{scoreAlias}}数</button>
