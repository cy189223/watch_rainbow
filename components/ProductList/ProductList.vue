<template>
	...
	<view class="title-container">
		<view class="text" v-if="classlist && classlist != ''" @tap="navtype = 0" :class="navtype === 0 ? 'cur' : ''">
			全部
		</view>
...
<script>
	export default {
		...
		mounted() {
			this.initData()
			this.$http('/shop/recommend-cateogries', 'GET', {
			}).then(res => {
...
<style lang="scss">
	.title-container {
		...
		.text {
			...
			&.cur {
				text-shadow: 1px 1px 0px #00d0ff, -2px 0px 0px #c41cef;
				font-size: 34rpx;
			}
		}
	}
