	...
	<view>
		...
		<view class="liushuibox" v-if="liushuilist.list">
			<view v-for="(item,index) in liushuilist.list" :key="index" style="padding-bottom: 5px;">
				...
				<view v-for="(ite,ind) in item.skus" :key="ind" class="mainbox">
					...
					<view class="flexbox">
						<view class="numbox"><image class="img" src="../../../static/zuanshi.png" mode=""></image><view>{{ite.score && ite.score != '' ? ite.score : '0'}}</view></view>
		...
		<view class="content-c">
			<view class="module-c" :style="'margin-top:' + item.style.margin_top + 'rpx; padding: 0 ' + item.style.margin + 'rpx;'" v-for="(item, index) in modules"  :key="index">
				<ActivityList :refreshCounter="refreshCounter"  :getNextPageCounter="getNextPageCounter" :ids="item.list" :module="item" v-show="item.type === 'activities' && navtype == item.activity_type" />
			</view>
		</view>
