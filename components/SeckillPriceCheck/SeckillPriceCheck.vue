<template>
    <view class="mask" @tap="cancel" @touchmove.stop>
        <view class="mask-content animated bounceInLeft" @tap.stop>
            ...
            <view class="desc">当前登陆用户需要符合以下条件才能享受此秒杀价:</view>
            <view class="rule-c">
            ...
            <view class="price-c" v-if="isInit">
...
export default {
    ...
    methods: {
        ...
        initOrder() {
            ...
            uni.showLoading();
            this.$http(`/seckills/${this.seckillUuid}/check-user-group`, 'POST').then((res) => {
                this.isInit = true;
                this.info = res.data.info;
                this.options = res.data.options;
                (this.isPassed = res.data.is_passed), uni.hideLoading();
            });
        },
        cancel() {
            this.$emit('cancel');
